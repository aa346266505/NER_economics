...
from models.bert_for_ner import BertCrfForNer,BertSpanForNer,BertSoftmaxForNer
...
parser.add_argument("--data_dir", default='processors/', type=str,
...
MODEL_CLASSES = {
    ...
    'bert_softmax': (BertConfig,BertSoftmaxForNer,BertTokenizer),
}

def train(args, train_dataset, model, tokenizer):
    ww = open('data_set/loss.txt','w',encoding='utf-8')
    """ Train the model """
...
def predict(args, model, tokenizer, prefix=""):
    ...
    results = []
    output_predict_file = os.path.join(pred_output_dir, prefix, "submit_b.txt")
...
def main():
    ...
    if 0 and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
    ...
    results = {}
    # args.do_eval
    if 0 and args.local_rank in [-1, 0]:
    ...
    # args.do_predict
    if 1 and args.local_rank in [-1, 0]:
